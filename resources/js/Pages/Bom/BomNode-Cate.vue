...
<div>  
  ...
  @mouseleave="hoveredId = null"
    ...
    >
      <!-- 展开收起最左边的指示箭头 -->
      <div
        class="w-5 h-5 flex mr-1.5 text-[#888] text-xs cursor-pointer"
      ...
      <div 
        class="flex rounded-lg bg-yellow-400 m-1 px-3 justify-center min-w-[60px] relative">
          ...
          >      
              {{ item.id === 0 ? 0 : (item.cateName || getCateLabelById(item.cateId) || "类别") }}
          ...
          >
              ...
              <input
                  ...
                  @keyup.enter="$emit('add-cate', item, newCateText);activeCateId = null;showCateInput = false"
                  @blur="closeAllMenu()"
                  v-focus=""
              />
              <!-- 类别添加下拉菜单 -->
      ...
      <div
        v-if="getFieldVisible('unit')"
        class="flex bg-red-400 m-1 px-2  rounded-sm items-center justify-center min-w-[60px]"
        >
        <DropdownSubMenu
          :model-value="activeUnitItemId === item.id"
          :unit-group-list="unitGroupList"
          @update:modelValue="(open) => handleUnitPopup(item, open)"
          @select="(key) => {
...
const props = defineProps({
  ...
  getCateLabelById:{type:Function,default: () => (cateId) => ''}
})
...
// 通过 cateId 获取类别名称
const getCateLabelById = (cateId) => {
  if (!cateId && cateId !== 0) return '类别'
  const found = props.cateOptionList.find(c => c.cateId === cateId)
  return found?.cateName || '类别'
}


const fullBomTree = ref(props.bomData)
const hoveredId = ref(null)//Hover背景控制
// 3. 基础局部响应式变量 + 自定义指令
...
const activeUnitItemId = ref(null)
...
let openUnitDropdownId,showCateInput,newCateText,inputRef,openAddInput,addNewCate,getRawTree
...
const handleUnitPopup = (item, open) => {
  if (item.lockField) return

  if (open) {
    activeUnitItemId.value = item.id
  } else if (activeUnitItemId.value === item.id) {
    activeUnitItemId.value = null
  }
}
// 修改 handleSelectCate（在子组件中处理选择）
const handleSelectCate = (item, option) => {
  item.cateId = option.cateId
  item.cateName = option.cateName
  activeCateId.value = null
  // 触发父组件更新
  emit('select-cate', item, option)
  emit('update:bom-data', [...props.bomData])
}
const chooseCate = (opt,rowItem)=>{
  if(!rowItem) return
  rowItem.cateName = opt.cateName
  activeCateId.value = null
}
// 点击页面任意地方关闭下拉
